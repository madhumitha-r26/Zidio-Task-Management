...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    fetch("https://zidio-task-management-serverside.vercel.app/users/verify", {
      method: "POST",
      headers: {
  ...
  useEffect(() => {
    if (user && user.email) {
      const fetchTasks = async () => {
        try {
          const response = await fetch(`https://zidio-task-management-serverside.vercel.app/tasks/${user.email}`, {
            method: "GET",
            credentials: "include",
          });
          if (!response.ok) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      if (editingIndex !== null) {
        const response = await fetch(`https://zidio-task-management-serverside.vercel.app/tasks/${tasks[editingIndex]._id}`, {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
          },
        ...
        const response = await fetch("https://zidio-task-management-serverside.vercel.app/tasks", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
  ...
  const handleDelete = async (index) => {
    try {
      const response = await fetch(`https://zidio-task-management-serverside.vercel.app/tasks/${tasks[index]._id}`, {
        method: "DELETE",
      });
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
  ...
  const toggleTaskCompletion = async (index) => {
    ...
    try {
      const response = await fetch(`https://zidio-task-management-serverside.vercel.app/tasks/${task._id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
